#include "asctec_autopilot/fdp.h"
#include <sstream>

using namespace std;
using namespace ros;

string Flocking::Concatenation(string s, int Num)
{
ostringstream ss;
ss << s << Num ;
...
void Flocking::Callback_GPS(const asctec_msgs::GPSDataConstPtr msg, int DroneNumero)
{
    ...
    cout <<"CallBack Drone : "<< Drone[DroneNumero].numero <<"Latitude : "<< Drone[DroneNumero].positionGeo.latitude << "\n";
    cout <<"CallBack Drone : "<< Drone[DroneNumero].numero <<"Longitude : "<< Drone[DroneNumero].positionGeo.longitude << "\n";
}
...
Drone[i].Pub_Waypoint_Input = Concatenation(Drone[i].Pub_Waypoint_Input, Drone[i].numero);
//cout << "WPin"<<Drone[i].Pub_Waypoint_Input << endl;
//cout.flush();
Drone[i].Sub_GPS = Concatenation(Drone[i].Sub_GPS, Drone[i].numero);
//cout <<  "Sub_GPS"<<Drone[i].Sub_GPS << "\n";
//cout.flush();
Drone[i].Sub_Current_Way = Concatenation(Drone[i].Sub_Current_Way, Drone[i].numero);
//cout <<  "Currentway"<<Drone[i].Sub_Current_Way << "\n";
//cout.flush();
Drone[i].Sub_ACK = Concatenation(Drone[i].Sub_ACK, Drone[i].numero);
//cout << "Ack"<<Drone[i].Sub_ACK << "\n";
//cout.flush();
Drone[i].Sub_IMU = Concatenation(Drone[i].Sub_IMU, Drone[i].numero);
//cout << "IMU"<<Drone[i].Sub_IMU << "\n";
//cout.flush();
...
//ROS_INFO("Marque 6 (boucle for terminee)");
    if(CodeSup > 0){

        MesDrones DroneTemp;
        DroneTemp.numero = CodeSup;
	//cout << "Drone Superieur : " << CodeSup << "\n";
        DroneTemp.hauteur = HauteurSup;
        Drone.push_back(DroneTemp);
        long unsigned int i = (Drone.size()-1);
Drone[i].Pub_Waypoint_Input = Concatenation(Drone[i].Pub_Waypoint_Input, Drone[i].numero);
//cout << "WPin"<<Drone[i].Pub_Waypoint_Input << endl;
//cout.flush();
Drone[i].Sub_GPS = Concatenation(Drone[i].Sub_GPS, Drone[i].numero);
...
subscriber_IMU_tab[i]=n.subscribe<asctec_msgs::IMUCalcData>(Drone[i].Sub_IMU,100, boost::bind(&Flocking::Callback_IMU,this,_1,i));
        ...
        EcartSup(i);
        cout << "Drone Superieur : " << Drone[i].numero << "instancie."<< "\n";
    }
    cout << "Il y a :" << Drone.size() << " drones \n";
    cout << "Instantiation Terminee." << "\n";
}
    ...
    {
	cout << "Latitude entree : " << pt.latitude << endl;
	cout << "Longitude entree : " << pt.longitude << endl;	

	double tempptlat;
	tempptlat = pt.latitude/10000000;
	double tempptlong ;
	tempptlong = pt.longitude/10000000;
        double a = 6378137;
        double e = 0.0818191910428158;
        double l = tempptlat*M_PI/180;
        double sinl = sin(l);
        coordinatesMerc out;
	//cout << "GeoToMerc" << Drone.size() << " drones \n";
        out.X = a*tempptlong*M_PI/180;
        out.Y = a*log(tan(l/2+M_PI_4)*pow((1-e*sinl)/(1+e*sinl),(e/2)));
        cout << "X sorti : " << out.X << endl;
	cout << "Y sorti : " << out.Y << endl;
	return out;
    ...
    {
	cout << "MercToGeo : Merc X entree : " << pt.X << endl;
	cout << "MercToGeo : Merc Y entree : " << pt.Y << endl;
        double a = 6378137;
        double e = 0.0818191910428158;
        double e2 = e*e;
        double e4 = e2*e2;
        ...
        out.longitude = round(((pt.X/a)*180/M_PI)*10000000);
        double ki = atanh(pt.Y/a);
        double phi = ki + d2*sin(2*ki) + d4*sin(4*ki) + d6*sin(6*ki) + d8*sin(8*ki);
        out.latitude = round((phi*180/M_PI)*10000000);
	cout << "MercToGeo : Latitude sortie : " << out.latitude << endl;
	cout << "MercToGeo : Longitude sortie : " << out.longitude << endl;
        return out;
    }
...
std::vector<int> Flocking::VectorParsing(string s)
{
	...
	//cout<< "VectorParsing"<<s << atoi(token.c_str()) << endl;
        s.erase(0, pos + delimiter.length());
    }
    return out;
}
...
long unsigned a = Vlati.size();
	    ...
	    //cout<< "Vlati de : "<< i << " est " << Vlati[i] << endl;
	    //cout<< "Vlongi de : "<< i << " est " << Vlongi[i] << endl;
...
void Flocking::new_waypoint(){
	...
	coordinatesMerc temps;
    Drone[0].destinationGeo = pt;
    Drone[0].angle = CapDestination(0);
    ChangeAngle(0, Drone[0].angle);
cout << "Drone numero : 0 Latitude : " << Drone[0].positionGeo.latitude << endl;
cout << "Drone numero : 0 Longitude : " << Drone[0].positionGeo.longitude << endl;
temps = geoToMerca(Drone[0].positionGeo);
cout << "Drone numero : 0 X : " << temps.X << endl;
cout << "Drone numero : 0 Y : " << temps.Y << endl;

long unsigned a = Drone.size();
    for (long unsigned int i=1; i< a; i++)
    {
coordinatesMerc temp;
        Drone[i].destinationGeo = CalcPosGeo(Drone[0].positionGeo, Drone[0].angle,i);
        Drone[i].angle = Drone[0].angle;
cout << "Drone numero : " << i << " Latitude : " << Drone[i].destinationGeo.latitude << endl;
cout << "Drone numero : " << i << " Longitude : " << Drone[i].destinationGeo.longitude << endl;
temp = geoToMerca(Drone[i].destinationGeo);
cout << "Drone numero : " << i << " X : " << temp.X << endl;
cout << "Drone numero : " << i << " Y : " << temp.Y << endl;
    }
