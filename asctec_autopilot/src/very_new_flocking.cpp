#include "asctec_autopilot/very_new_flocking.h"
#include <sstream>
...
{   
	...
	cout << "caca"<< "\n";
	cout.flush();
	cout << Drone[i].Pub_Waypoint_Input << "\n";
	cout.flush();
    }
...
cout.flush();
 ...
 ROS_INFO("Codesup4: %i",CodeSup);
    for (long unsigned int i=0; i<= a; i++){
//PROBLEME ICI
ROS_INFO("i: %i",i);
Drone[i].Pub_Waypoint_Input = Concatenation(Drone[i].Pub_Waypoint_Input, i);

            //Drone[i].Pub_Waypoint_Input+=Drone[i].numero+"";
	cout << "WPin"<<Drone[i].Pub_Waypoint_Input << endl;
cout.flush();
ROS_ERROR("Codesup5: %s",Drone[i].Sub_GPS.c_str());
Drone[i].Sub_GPS = Concatenation(Drone[i].Sub_GPS, i);
cout.flush();
          //  Drone[i].Sub_GPS+=Drone[i].numero+"";
            //Drone[i].Sub_Current_Way+=Drone[i].numero+"";
ROS_ERROR("Codesup5: %s",Drone[i].Sub_GPS.c_str());
Drone[i].Sub_Current_Way = Concatenation(Drone[i].Sub_Current_Way, i);
cout <<  "Currentway"<<Drone[i].Sub_Current_Way << "\n";
cout.flush();
            //Drone[i].Sub_ACK+=Drone[i].numero+"";
Drone[i].Sub_ACK = Concatenation(Drone[i].Sub_ACK, i);
cout << "Ack"<<Drone[i].Sub_ACK << "\n";
cout.flush();
           // Drone[i].Sub_IMU+=Drone[i].numero+"";
Drone[i].Sub_IMU = Concatenation(Drone[i].Sub_IMU, i);
...
cout.flush();
		...
		subscriber_IMU_tab[i]=n.subscribe<asctec_msgs::IMUCalcData>(Drone[i].Sub_IMU,100, boost::bind(&Flocking::Callback_IMU,this,_1,i));
 		ROS_INFO("Marque 4");
            ...
            //cout << "Drone1 n  " << Drone[i].numero << " instancie. \n";

            //********************* A FINIR ******************
            // Remplir les décalages des drone en fonction de la formation
            // Ainsi que les hauteurs désirées
        }
 ROS_INFO("Codesup7: %i",CodeSup);
    if(CodeSup > 0){

ROS_INFO("Codesup8: %i",CodeSup);
        MesDrones DroneTemp;
